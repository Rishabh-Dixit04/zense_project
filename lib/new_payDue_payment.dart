...
class NewPayDuePayment extends StatefulWidget {
  Map element;
  NewPayDuePayment(this.element,{Key? key}) : super(key: key){}

  @override
  State<NewPayDuePayment> createState() => _NewPayDuePaymentState();
}

class _NewPayDuePaymentState extends State<NewPayDuePayment> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var doc in documents) {
      if (doc['name'] == accountName) {
        ...
        if (amount<=currentMoney){
          _payDuesReference.doc(widget.element['id']).delete();
    ...
    return Form(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            Text('Pay amount',style: TextStyle(fontSize: 20)),
            SizedBox(height: 20,),
            Dropdownforaccounts(),
